...
var Store = require('../datastores/redis');

var NS = 'querycache_test';

test('Honour the MAX_ENTRIES bound', function (t) {
  var maxEntries = 2;
  var cache = new Store({maxEntries: maxEntries, namespace: NS});
  ...
  async.series(setCache, function (err) {
    if (err) throw err;
    var entries = cache._cacheKeys.length;
...
test('When MAX_ENTRIES is reached, remove oldest entry', function (t) {
  var cache = new Store({maxEntries: 2, namespace: NS});
  async.series([
